...
RUN cp -a "$(swift build --package-path /build -c release --show-bin-path)/." ./

# Copy resources to staging area
RUN find -L "$(swift build --package-path /build -c release --show-bin-path)/" -regex '.*\.resources$' -exec cp -a {} ./ \;
...
COPY --from=build --chown=vapor:vapor /staging /prod
